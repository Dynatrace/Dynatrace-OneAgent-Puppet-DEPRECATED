# Get the installed version for the oneagent agent.
#
...
Facter.add(:oneagent_installed_version) do
  setcode do
    version = nil
    file = nil
    os_key = nil
    if Facter.value('kernel').eql? 'windows'
      if Facter.value('architecture').eql? 'x64'
        file = 'C:/Program Files (x86)/dynatrace/oneagent/agent/conf/binaries.json'
    ...
    if nil != file
      begin
        ...
        Puppet.debug("Unable to determine installed oneagent agent version - #{ex}")
      end
    end # kernel check
